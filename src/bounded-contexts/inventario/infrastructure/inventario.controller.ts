...
export class InventarioController {
  ...
  @Post('transferir-stock-lote')
  @Roles(Rol.ROL_ADMIN, Rol.ROL_BODEGUERO)
  async transferirStockLote(@Body() dto: TransferirStockLoteDto, @Req() req: any) {
    const origenTenantId = req.user.tenantId;
    if (!origenTenantId) {
      throw new BadRequestException('No se determinó la sucursal de origen');
    }
    ...
    if (!dto.items || !Array.isArray(dto.items) || dto.items.length === 0) {
      throw new BadRequestException('Debe incluir al menos una talla para despachar');
    }

    const itemsValidos = dto.items.filter((i) => i.cantidad > 0);
    if (itemsValidos.length === 0) {
      throw new BadRequestException('La cantidad total a despachar debe ser mayor a 0');
    }
    ...
    // Validar stock para todas las tallas
    for (const item of itemsValidos) {
      const stockOrigen = productoOrigen.stockByTalla.find((s) => s.tallaId === item.tallaId);
      if (!stockOrigen || stockOrigen.quantity < item.cantidad) {
        const numTalla = stockOrigen?.talla?.numero || 'N/A';
        throw new BadRequestException(
          `Stock insuficiente en Talla ${numTalla}. Disponible: ${stockOrigen?.quantity || 0} pares`,
        );
      }
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      // Buscar o crear modelo en destino
      const destSuffix = destinoTenant.id.substring(0, 4).toUpperCase();
      let modeloDestino = await tx.productModel.findFirst({
        where: {
          tenantId: destinoTenant.id,
      ...
      for (const item of itemsValidos) {
        ...
        await tx.stockByTalla.update({
          where: { id: stockOrigen.id },
          data: { quantity: stockOrigen.quantity - item.cantidad },
        });
        ...
        if (stockDestino) {
          await tx.stockByTalla.update({
            where: { id: stockDestino.id },
            data: { quantity: stockDestino.quantity + item.cantidad },
          });
        } else {
          await tx.stockByTalla.create({
            data: {
      ...
      const totalPares = itemsValidos.reduce((acc, i) => acc + i.cantidad, 0);
      await tx.auditLog.create({
        data: {
          tenantId: origenTenantId,
          userId: req.user.sub,
          ...
          entidadId: productoOrigen.id,
          detalles: {
            tipoOperacion: 'TRANSFERENCIA_STOCK_LOTE',
            modelo: productoOrigen.model.name,
            color: productoOrigen.color,
            totalPares,
            items: itemsValidos.map((i) => ({ tallaId: i.tallaId, cantidad: i.cantidad })),
            origen: origenTenant.name,
            destino: destinoTenant.name,
            motivo: dto.motivo,
          } as any,
    ...
    return {
      ok: true,
      message: `Se despacharon ${totalPares} pares de ${productoOrigen.model.name} (${productoOrigen.color}) desde ${origenTenant.name} hacia ${destinoTenant.name} exitosamente.`,
    };
  }
